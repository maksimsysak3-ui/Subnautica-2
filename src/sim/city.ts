...
/** Two vec4s per instance, matching struct Box in box.wgsl. */
...
/** PCG-style integer hash. Same input, same output, on every machine. */
...
export function makeCity(): City {
  ...
  for (let gx = 0; gx < GRID; gx++) {
    for (let gz = 0; gz < GRID; gz++) {
      ...
      const spike = hash(gx, gz, 7);
      const base = 9 + hash(gx, gz, 3) * 13;
      ...
      const j = (hash(gx, gz, 11) - 0.5) * 0.06;
      const cr = 0.20 + t * 0.34 + j;
      const cg = 0.24 + t * 0.36 + j;
      const cb = 0.30 + t * 0.38 + j;

      out.push(cx, cz, halfX, halfZ, height, cr, cg, cb);
    }
  }
