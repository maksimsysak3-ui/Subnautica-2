/**
 * Housing: seventeen buildings across three densities.
 *
...
function detachedHouse(lod: number): MeshBuilder {
  ...
  if (fine) {
    ...
    m.box([wx1 + 0.1, 0, wz1 - 2.4], [wx1 + 3.4, 0.16, wz1], MAT.CONCRETE);
...
function bungalow(lod: number): MeshBuilder {
  ...
  m.gable([-x, wall, -z], [x, wall, z], pitch(d), 'x', MAT.TILE, MAT.PLASTER);
  ...
  if (fine) {
    ...
    frontage(m, -x, x + 4.4, z, 5, { trees: 3, planters: 3, bollards: 6 });
    ...
    m.gable([-x + 0.5, 2.1, -z - 3.5], [-x + 3.3, 2.1, -z - 0.9], 0.7, 'x', MAT.TILE, MAT.TRIM);
...
function duplex(lod: number): MeshBuilder {
  ...
  m.box([-x, 0, -z], [x, wall, z], MAT.BRICK, { roof: MAT.TRIM });
  m.gable([-x, wall, -z], [x, wall, z], pitch(d), 'x', MAT.TILE, MAT.BRICK);
...
function cottage(lod: number): MeshBuilder {
  ...
  m.box([-x, 0, -z], [x, wall, z], MAT.PLASTER, { roof: MAT.TRIM });
  m.gable([-x, wall, -z], [x, wall, z], ridge, 'z', MAT.TILE, MAT.PLASTER);
  ...
  if (medium) {
    ...
    m.quad([x, 2.45, 2.6], [x + 2.6, 2.05, 2.6], [x + 2.6, 2.05, -1.6], [x, 2.45, -1.6], MAT.TILE);
  ...
  if (fine) {
    ...
    frontage(m, -x - 1.6, x + 2.8, z + 0.8, 17, { trees: 3, planters: 3, bollards: 4, depth: 2.0 });
...
function largeHouse(lod: number): MeshBuilder {
  ...
  if (medium) {
    ...
    m.gable([-x, wall - 0.3, z - 0.4], [-x + 4.2, wall - 0.3, z + 3.0], pitch(4.2), 'x', MAT.TILE, MAT.BRICK);
    eaves(m, -x, -z, x, z, wall);
...
function courtyardBlock(lod: number): MeshBuilder {
  ...
  if (medium) {
    ...
    tree(m, 0, -z + t + 4.0, 6.0, 41);
    tree(m, -3.6, z - 3.0, 5.2, 43);
    tree(m, 3.6, z - 3.0, 5.2, 47);
  }
  if (fine) {
...
function cornerBlock(lod: number): MeshBuilder {
  ...
  if (fine) {
    shopfront(m, { axis: 'z', sign: 1, plane: z }, -x + 0.6, x - 4.2, { bays: 3, doorBay: 1, head: 3.6 });
...
function pointTower(lod: number): MeshBuilder {
  ...
  if (fine) {
    ...
    shopfront(m, { axis: 'z', sign: 1, plane: z }, -x + 1.0, x - 1.0,
      { bays: 4, doorBay: 1, head: 4.4, brandFascia: false });
...
function slabTower(lod: number): MeshBuilder {
  ...
  if (fine) {
    ...
    shopfront(m, { axis: 'z', sign: 1, plane: z }, -x + 2.0, x - 2.0,
      { bays: 6, doorBay: 2, head: 4.2, brandFascia: false });
  }
  return m;
}
...
function glassTower(lod: number): MeshBuilder {
  ...
  if (fine) {
    ...
    shopfront(m, { axis: 'z', sign: 1, plane: z }, -x + 1.2, x - 1.2,
      { bays: 5, doorBay: 2, head: 5.2, brandFascia: false });
  }
  return m;
}
...
function twinPodium(lod: number): MeshBuilder {
  ...
  if (medium) {
    ...
    tree(m, -6.0, z - 3.4, 4.6, 201);
    tree(m, 6.0, z - 3.4, 4.6, 203);
  ...
  if (fine) {
    ...
    shopfront(m, { axis: 'z', sign: 1, plane: z }, -x + 1.5, x - 1.5,
      { bays: 6, doorBay: 3, head: 4.6, brandFascia: false });
...
export const RESIDENTIAL: AssetDef[] = [
  ...
  { id: 'res.mid.mixed', name: 'Flats over shops', zone: 'residential', density: 'medium', variant: 'sculpted', footprint: [3, 3], height: 17.2, sim: home(28), note: 'Two retail units at the base, four floors of flats with balconies over.', build: mixedUse },
