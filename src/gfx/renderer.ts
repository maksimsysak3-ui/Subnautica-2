...
import { poolSiblings } from '../sim/inventory';
...
export class Renderer {
  ...
  private loadWorld(layouts: Layouts, old?: WorldRes): WorldRes {
    ...
    this.city = city;
    // What the city is drawing from, minus what it has already placed: the
    // list `warmOne` works through between now and the next edit.
    const census = city.population;
    this.prebake = poolSiblings((i) => (census[i] ?? 0) > 0);
    this.prebakeAt = 0;
    this.uploadSurface();
  ...
  /**
   * Prototypes worth generating before anything asks for them.
   *
   * Generating a building is the most expensive single thing in the project --
   * three meshes, each with its ambient occlusion baked -- and it happened at
   * the worst possible moment: inside the rebuild that follows an edit, because
   * that is when the spawner first asks for a kind of building the city has not
   * had before. Drawing a road through a zoned quarter could introduce nine of
   * them at once and the frame stopped for a fifth of a second.
   *
   * So the rebuild no longer is the first time anything asks. After each one
   * the pools the city is drawing from are swept for members it has not placed
   * yet, and those are generated one per frame while the frame has room. What a
   * player feels is a road that builds at the same speed as the last one.
   */
  private prebake: string[] = [];
  private prebakeAt = 0;
  /** When the next one may be generated. */
  private prebakeNext = 0;
  /**
   * Where the warming stops.
   *
   * Every prototype generated ahead of need is geometry in the arena that
   * nothing is drawing yet, and the arena lives on the GPU. Sixty-four
   * megabytes is roughly twice what a full city places, which is room for every
   * pool it is drawing from and no room for the rest of the library.
   */
  private static readonly PREBAKE_CAP = 64 * 1024 * 1024;

  /**
   * Generates the next waiting prototype. False when there is none left.
   *
   * One at a time, because a mesh cannot be generated by halves: the atlas
   * builds all three levels together so that they share one quantisation frame.
   * A single one is a few milliseconds for a house and fifteen for a tower --
   * against a frame that skips, rather than an edit that freezes.
   *
   * Public so that a loading screen, which has nothing else to do, can work
   * through the queue as fast as it likes.
   */
  warmNext(): boolean {
    if (this.prebakeAt >= this.prebake.length) return false;
    if (this.atlas.byteLength >= Renderer.PREBAKE_CAP) {
      this.prebakeAt = this.prebake.length;
      return false;
    }
    const id = this.prebake[this.prebakeAt++];
    try {
      this.atlas.bakeAll(id);
    } catch (err) {
      // A generator that throws is a bug in that generator, and it will throw
      // again when the city places it for real and the error is attributable.
      // Warming is not the place to surface it.
      log.warn('render', `could not warm ${id}: ${String(err)}`);
    }
    return true;
  }

  /** What is still waiting to be generated ahead of need. */
  get warmList(): readonly string[] { return this.prebake.slice(this.prebakeAt); }

  private warmOne(now: number): void {
    if (this.prebakeAt >= this.prebake.length) return;
    // Ten a second at most, so the warming is a few per cent of the frame
    // budget on any machine rather than a fifteen-millisecond mesh on every
    // one of them. A sweep of a city's pools finishes inside ten seconds.
    if (now < this.prebakeNext) return;
    // Not while the GPU is already behind, and not while the machine is
    // struggling: warming is an optimisation and must never be the thing that
    // costs a frame.
    if (this.inFlight >= MAX_IN_FLIGHT) return;
    if (this.stats.frameMs > 20) return;
    this.prebakeNext = now + 100;
    this.warmNext();
  }

  /**
   * Adds prototypes to the warming queue, nearest first.
   *
   * Called by the drawers: a service or a signature building is placed one at a
   * time and the drawer knows which one is in hand, which is a far better guess
   * than a whole branch of the library.
   */
  warmAssets(ids: readonly string[]): void {
    // At the front of what is left, because these are what the player is about
    // to place rather than what the city might grow into.
    const rest = this.prebake.slice(this.prebakeAt);
    this.prebake = ids.concat(rest.filter((id) => !ids.includes(id)));
    this.prebakeAt = 0;
    // Straight away: the player has just picked this out of a drawer.
    this.prebakeNext = 0;
  }
  ...
  private frame(now: number): void {
    ...
    // Last, after the work is submitted: the GPU has a frame's worth of drawing
    // to get on with and the CPU has nothing to do until it is done.
    this.warmOne(now);
  }
