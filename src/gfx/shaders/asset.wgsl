...
struct Scene {
   ...
   * view is open. y = how far it is drained towards grey, while a surface one
   * is. See `bury` and `drain` in common.wgsl, which this matches by hand --
   * this shader cannot include that file, because both want group 1.
   */
  view        : vec4f,
};
...
fn fs(in : VSOut) -> @location(0) vec4f {
  ...
  // for why this shader cannot include that file.
