...
const TILE = Number(process.env.TILE ?? 512);          // 512 * 4 bytes is a multiple of the 256-byte row alignment
const TILE_H = Number(process.env.TILE_H ?? 576);
...
const result = await page.evaluate(async ({ shader, registry, TILE, TILE_H, COLS, LOD, SHADOW, ONLY, STREET, ZOOM, YAW, PITCH, AIM, DIST }) => {
  ...
  const G = 400;
  // Thirteen floats a vertex: position, normal, material, occlusion, tint,
  // surface coordinates, part key, packed colour.
  const gv = new Float32Array([
    -G, 0, -G, 0, 1, 0, 8, 1, 0, 0, 0, 0, 0,   G, 0, -G, 0, 1, 0, 8, 1, 0, 0, 0, 0, 0,
     G, 0,  G, 0, 1, 0, 8, 1, 0, 0, 0, 0, 0,  -G, 0,  G, 0, 1, 0, 8, 1, 0, 0, 0, 0, 0,
  ]);
