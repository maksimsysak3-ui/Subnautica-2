...
function play(difficulty) {
  ...
  for (let day = 1; day <= DAYS; day++) {
    ...
    if (day % 10 === 0 || day === 1) {
      ...
      log.push({
        day, pop, level: p.level, balance: Math.round(world.budget.balance), net: Math.round(e.net), gin: Math.round(e.income), gout: Math.round(e.spending), svc: Math.round(e.services), rd: Math.round(e.roads), tr: Math.round(e.transit), pol: Math.round(e.policies),
...
for (const d of WHICH) {
  ...
  console.log('  day    pop  lvl   balance    net/wk     in/wk    out/wk  services     roads  happy  fires crimes missed');
  for (const l of r.log) {
    console.log(`  ${String(l.day).padStart(3)} ${String(l.pop).padStart(6)} ${String(l.level).padStart(4)}`
      + ` ${String(l.balance).padStart(9)} ${String(l.net).padStart(9)} ${String(l.gin).padStart(9)} ${String(l.gout).padStart(9)} ${String(l.svc).padStart(9)} ${String(l.rd).padStart(9)} ${String(l.happy).padStart(5)}%`
